...
如果您在运行中遇到任何问题，请通过如下方式直接将报错信息截图发送到邮箱**xarnudvilas@gmail.com**或添加微信**molai2048**/QQ**981758936**, 也可在本仓库页面提交issue，我会在第一时间进行修复。
...
对于直接预测的数据(调用`start`脚本)，需要您准备`csv`文件，其中必须包含唯一名为`SMILES`的字段，具体可参考根目录下的`raw.csv`

对于您自己准备用来训练和验证的数据集，需要包含的字段为`SMILES`(分子)，`SIF`(0/1,在`SIF`下是否稳定)，`SGF`(0/1,在`SGF`下是否稳定)。具体可参考根目录下的`sample_train.csv`
